    ...
    <div ref={setNodeRef} style={style} {...attributes} {...listeners} className="relative group cursor-pointer">
      <Card className={cn(
        ...
        <div className="flex flex-col items-start justify-end pt-1">
          {taskEndDate && (
            <div className={cn(
              "flex items-center text-xs w-full overflow-hidden", // Added w-full overflow-hidden
              isOverdue ? "text-destructive" : "text-muted-foreground"
            )}>
              <Calendar className="w-3 h-3 mr-1 flex-shrink-0" /> {/* Added flex-shrink-0 */}
              <span className="line-clamp-1 flex-grow min-w-0">{format(taskEndDate, 'PPP', { locale: currentLocale })}</span> {/* Added flex-grow min-w-0 */}
              {isOverdue && (
                <Badge variant="destructive" className="ml-1 px-1 py-0.5 text-[0.6rem] flex-shrink-0">
          ...
          {domainTitle && (
            <Badge variant="outline" className="mt-1 font-normal text-[0.6rem] px-1 py-0.5 line-clamp-1 w-full overflow-hidden"> {/* Added w-full overflow-hidden */}
              {domainTitle}
            </Badge>
          )}
        </div>
